    ...
    return (
        <div className="overflow-hidden rounded-xl border border-slate-100 bg-white shadow-xs dark:border-slate-900 dark:bg-slate-950">
            ...
            {(title || (legend && legend.length > 0)) && (
                <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-900 bg-slate-50/20 dark:bg-slate-900/10">
                    {title && (
                            ...
                            {legend.map((item, idx) => {
                                ...
                                switch (item.variant) {
                                    case 'amber':
                                        ...
                                        iconWrapperClass = 'border-amber-250 bg-amber-100 text-amber-700 dark:border-amber-900/30 dark:bg-amber-950/20 dark:text-amber-400';
                                        break;
                                    case 'blue':
                                        badgeClass = 'bg-blue-50/50 dark:bg-blue-950/10 border-blue-100 dark:border-blue-900/20 text-blue-800 dark:text-blue-300';
                                        iconWrapperClass = 'border-blue-250 bg-blue-100 text-blue-700 dark:border-blue-900/30 dark:bg-blue-950/20 dark:text-blue-400';
                                        break;
                                    case 'rose':
                                        badgeClass = 'bg-rose-50/50 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/20 text-rose-800 dark:text-rose-300';
                                        iconWrapperClass = 'border-rose-250 bg-rose-100 text-rose-700 dark:border-rose-900/30 dark:bg-rose-950/20 dark:text-rose-400';
                                        break;
                                    case 'emerald':
                                        badgeClass = 'bg-emerald-50/50 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-900/20 text-emerald-800 dark:text-emerald-300';
                                        iconWrapperClass = 'border-emerald-250 bg-emerald-100 text-emerald-700 dark:border-emerald-900/30 dark:bg-emerald-950/20 dark:text-emerald-400';
                                        break;
                                    case 'indigo':
                                        badgeClass = 'bg-indigo-50/50 dark:bg-indigo-950/10 border-indigo-100 dark:border-indigo-900/20 text-indigo-850 dark:text-indigo-300';
                                        iconWrapperClass = 'border-indigo-250 bg-indigo-100 text-indigo-700 dark:border-indigo-900/30 dark:bg-indigo-950/20 dark:text-indigo-400';
                                        break;
                                    default:
                                ...
                                return (
                                    <div key={idx} className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border ${badgeClass}`}>
                                        <span className={`flex size-5.5 items-center justify-center rounded-md border ${iconWrapperClass}`}>
                                            {React.createElement(item.icon, { className: 'size-3' })}
                                        </span>
                    ...
                    <tbody className="divide-y divide-slate-50 font-semibold text-slate-700 dark:divide-slate-900 dark:text-slate-300">
                        {data.length === 0 ? (
                            <tr>
                                <td colSpan={columns.length} className="px-6 py-16 text-center">
                                    <div className="flex flex-col items-center justify-center">
                                        <div className="mb-4 flex size-14 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 border border-slate-100 shadow-3xs dark:bg-slate-900 dark:border-slate-800">
                                            {React.createElement(emptyState.icon, { className: 'size-6 text-slate-400' })}
                                        </div>
                                        <h3 className="text-sm font-bold text-slate-800 dark:text-white">{emptyState.title}</h3>
                                        <p className="mt-1 text-xs text-slate-550 max-w-2xl leading-relaxed dark:text-slate-400">
                                            {emptyState.description}
                                        </p>
                                        {emptyState.action && (
                                            <div className="mt-5">{emptyState.action}</div>
                                        )}
                                    </div>
                                </td>
                            </tr>
                            ...
                            data.map((item, rowIdx) => (
                                <tr key={rowIdx} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition">
                    ...
                    {totalPages > 1 && onPageChange && (
                        <div className="flex items-center gap-1.5">
                            <Button
                                ...
                                className="h-8 px-3 font-bold text-xs shadow-3xs cursor-pointer focus:outline-none dark:border-slate-800 dark:bg-slate-900 dark:text-slate-350 dark:hover:bg-slate-850"
                            >
                                Previous
                            </Button>
