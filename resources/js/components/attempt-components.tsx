...
export function StatusBadge({ status }: { status: string }) {
    ...
    if (normalized === 'pass' || normalized === 'passed') {
        badgeClass = 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-450 border border-emerald-200 dark:border-emerald-900/30';
        StatusIcon = CheckCircle2;
        text = 'Pass';
    } else if (normalized === 'fail' || normalized === 'failed') {
        badgeClass = 'bg-rose-50 text-rose-800 dark:bg-rose-950/20 dark:text-rose-450 border border-rose-200 dark:border-rose-900/30';
        StatusIcon = XCircle;
        text = 'Fail';
    } else { // Completed
        badgeClass = 'bg-blue-50 text-blue-800 dark:bg-blue-950/20 dark:text-blue-450 border border-blue-200 dark:border-blue-900/30';
        StatusIcon = CheckCircle2;
        text = 'Completed';
    }

    return (
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide ${badgeClass}`}>
        ...
        <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
                <div className="flex items-baseline gap-1">
                    <span className={`text-xs font-black shrink-0 ${scoreColor}`}>
                        {score}%
                    </span>
                    {fraction && (
            ...
            {categoryScores && categoryScores.length > 0 && (
                <div className="flex max-w-[24rem] flex-wrap gap-1.5">
                    {categoryScores.map((cat) => (
                        <span
                            key={cat.name}
                            className="inline-flex items-center gap-1 rounded-md border border-border bg-slate-50 px-1.5 py-0.5 text-[9px] font-bold text-muted-foreground dark:bg-slate-900 dark:border-slate-800"
