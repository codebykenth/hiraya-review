...
const renderFormattedText = (text: string) => {
    ...
    const formatNumberedLists = (inputText: string) => {
        ...
        const renderRichParagraph = (paraText: string, defaultClass: string = "text-slate-650 leading-relaxed text-base font-medium") => {
            ...
            const renderTokenContent = (token: string) => {
                ...
                if (token.includes('->') || token.includes('=>')) {
                    ...
                    return (
                        <span className="inline-flex items-center gap-1 mx-1 my-0.5 bg-slate-50 border border-slate-200/80 px-2 py-1 rounded-xl shadow-3xs hover:bg-slate-100/55 transition">
                            {variables.map((v, idx) => (
                    ...
                    {categories.map(cat => {
                        ...
                        return (
                            <div 
                                ...
                                className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-150 bg-white p-6 shadow-sm cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:border-slate-800 dark:bg-slate-950"
                            >
                                <div>
                                    <div className="flex items-start justify-between">
                                        ...
                                        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-extrabold text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                            📝 {actualCount} Qs
                                        </span>
                                    </div>

                                    <h3 className="mt-5 font-heading text-lg font-bold text-slate-850 transition group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400">
                                        {cat.name}
                                    </h3>
                                ...
                                <div className="mt-6 flex flex-wrap gap-1.5">
                                    {cat.subcategory.map(sub => (
                                        <span 
                                            key={sub.id}
                                            className="rounded-lg border border-slate-100 bg-slate-50/50 px-2 py-0.5 text-[9px] font-semibold text-slate-500 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-400"
                                        >
                                            {sub.name}
                                        </span>
                                    ))}
    ...
    const renderConfig = () => {
        ...
        return (
            <div className="flex flex-col gap-6">
                ...
                {isRetakeConfig && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900/30 dark:bg-amber-950/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-semibold text-amber-800 dark:text-amber-350 shadow-2xs">
                        ...
                        <button 
                            ...
                            className="rounded-lg bg-amber-100 hover:bg-amber-200 px-3 py-1.5 font-bold text-amber-900 dark:bg-amber-950 dark:hover:bg-amber-900 dark:text-amber-300 transition shrink-0 cursor-pointer focus:outline-none"
                        >
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="flex flex-col gap-5 lg:col-span-2">
                        ...
                        <div className="rounded-xl border border-slate-150 bg-white p-5 shadow-2xs dark:border-slate-800 dark:bg-slate-950">
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-1 rounded-full px-4.5 py-2 text-xs font-bold transition focus:outline-none ${
                                                ...
                                                isSelected
                                                    ...
                                                    : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 dark:bg-slate-900 dark:text-slate-400 dark:border-slate-800 dark:hover:bg-slate-800/50'
                                            }`}
                                        >
                                            {sub.name}
                        ...
                        <div className="rounded-xl border border-slate-150 bg-white p-5 shadow-2xs dark:border-slate-800 dark:bg-slate-950">
                            ...
                            <div className="flex flex-wrap gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`rounded-lg px-4 py-2 text-xs font-bold transition focus:outline-none border cursor-pointer ${
                                                ...
                                                isSelected
                                                    ...
                                                    : 'bg-slate-50/50 text-slate-600 border-slate-200 hover:bg-slate-100 dark:bg-slate-900 dark:text-slate-400 dark:border-slate-800 dark:hover:bg-slate-800'
                                            }`}
                                        >
                                            {count}
                                        </button>
                                ...
                                {/* Custom number input */}
                                <div className={`flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50/20 px-3 py-1 text-xs focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition dark:border-slate-800 dark:bg-slate-900/30 ${
                                    isRetakeConfig ? 'opacity-60 pointer-events-none select-none' : ''
                                }`}>
                                    <span className="text-slate-400 font-bold shrink-0">Custom:</span>
                                    <input
                                        type="number"
                                        min={1}
                                        disabled={isRetakeConfig}
                                        ...
                                        className="w-14 bg-transparent font-bold text-slate-700 focus:outline-none dark:text-slate-350 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        {/* 3. Language Selector */}
                        <div className="rounded-xl border border-slate-150 bg-white p-5 shadow-2xs dark:border-slate-800 dark:bg-slate-950">
                            <span className="text-[10px] font-extrabold tracking-wider text-slate-400 uppercase block mb-3.5">
                                🌐 Language
                            </span>
                            <div className="flex flex-wrap gap-6">
                                {['English', 'Filipino', 'Both'].map(lang => {
                                    ...
                                    return (
                                        <label key={lang} className={`flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-700 dark:text-slate-350 ${
                                            isRetakeConfig ? 'opacity-60 pointer-events-none select-none' : ''
                                        }`}>
                                            <input 
                                                type="radio" 
                                                ...
                                                className="size-4 text-blue-600 border-slate-300 focus:ring-blue-500 dark:bg-slate-900 dark:border-slate-800"
                                            />
                        ...
                        <div className="rounded-xl border border-slate-150 bg-white p-5 shadow-2xs dark:border-slate-800 dark:bg-slate-950">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                >
                                    <div className={`rounded-lg p-2 ${isTimed ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-900 dark:text-slate-400'}`}>
                                        <Timer className="size-4" />
                                    </div>
                                    <div>
                                        <span className="block text-xs font-black text-slate-850 dark:text-white">Timed Practice</span>
                                        <span className="mt-1 block text-[10px] text-slate-400 dark:text-slate-450 leading-normal">
                                ...
                                >
                                    <div className={`rounded-lg p-2 ${!isTimed ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-900 dark:text-slate-400'}`}>
                                        <Hourglass className="size-4" />
                                    </div>
                                    <div>
                                        <span className="block text-xs font-black text-slate-850 dark:text-white">Untimed Practice</span>
                                        <span className="mt-1 block text-[10px] text-slate-400 dark:text-slate-450 leading-normal">
                    ...
                    <div>
                        <div className="rounded-2xl border border-slate-150 bg-white p-6 shadow-md dark:border-slate-800 dark:bg-slate-950 flex flex-col justify-between h-fit">
                            <div>
                                <div className="flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
                                    <span className="font-heading text-base font-bold text-slate-850 dark:text-white">
                                        Drill Summary
                                    </span>
                                ...
                                <div className="mt-5 flex flex-col gap-4 text-xs">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="font-bold text-slate-700 dark:text-slate-300">{selectedCategory.name}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="font-bold text-slate-700 dark:text-slate-300">{language}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                            ...
                            <div className="mt-8">
                                ...
                                <span className="mt-3 block text-center text-[10px] text-slate-400">
